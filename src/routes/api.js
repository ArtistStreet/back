...
const hasAllowedVideoExt = (name = "") =>
  /\.(mp4|mov|webm|mkv|avi)$/i.test(String(name));
...
const productVideoUpload = multer({
  storage: multer.diskStorage({
    ...
    filename: (_req, file, cb) =>
      cb(null, buildDiskFilename("product-video", file)),
  }),
...
const reviewMediaUpload = multer({
  storage: multer.diskStorage({
    ...
    filename: (_req, file, cb) =>
      cb(null, buildDiskFilename("review-media", file)),
  }),
  limits: { fileSize: 200 * 1024 * 1024 },
  fileFilter: (_req, file, cb) => {
  ...
  return async (req, res, next) => {
    ...
    try {
      ...
      const preferred = String(
        req.headers["x-preferred-image-format"] || "",
      ).toLowerCase();
...
router.post(
  ...
  uploadAvatarRaw,
  optimizeAvatar,
  authController.uploadShopAvatar,
);
router.post(
  "/auth/shop-cover",
  protect,
  uploadShopCoverRaw,
  optimizeShopCover,
  authController.uploadShopCover,
);
  ...
  (req, res) => {
    ...
    const sources = {
      avif: optimized.avifFilename
        ? new URL(
            `/uploads/products/${optimized.avifFilename}`,
            baseUrl,
          ).toString()
        : imageUrl,
      webp: optimized.webpFilename
        ? new URL(
            `/uploads/products/${optimized.webpFilename}`,
            baseUrl,
          ).toString()
        : imageUrl,
    };
...
router.post(
  ...
  protect,
  isSeller,
  handleMulterUpload(productVideoUpload),
  (req, res) => {
    if (!req.file) return res.status(400).json({ message: "No file uploaded" });
    const baseUrl = `${req.protocol}://${req.get("host")}`;
...
router.post(
  ...
  protect,
  handleMulterUpload(reviewMediaUpload),
  (req, res) => {
    ...
    const images = imageFiles.map((file) =>
      new URL(`/uploads/reviews/${file.filename}`, baseUrl).toString(),
    );
